    ...
    def find(self, *args):
        ...
        if target:
            ...
            while 1:
                ...
                self.text.tag_add('found', foreground = 'white', background='blue' )
                idx = lastidx
...
def main(root, text, menuBar):
    ...
    objEdit.righClick.add_command(label="Copy", command=objEdit.copy)
    objEdit.righClick.add_command(label="Cut", command=objEdit.cut)
    objEdit.righClick.add_command(label="Paste", command=objEdit.paste)
    objEdit.righClick.add_separator()
    objEdit.righClick.add_command(label="Select All", command=objEdit.selectAll)
    objEdit.righClick.bind("<Control-q>", objEdit.selectAll)
